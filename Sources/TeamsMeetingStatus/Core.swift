...
public enum AppPaths {
    public static var applicationSupport: URL { FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support/MSTeamsStatusSender", isDirectory: true) }
    ...
    public static var logDirectory: URL { FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/MSTeamsStatusSender", isDirectory: true) }
...
public struct HTTPSender: StatusSending {
    ...
    public func send(_ state: MeetingState) async throws {
        ...
        case .local(let base, let token):
            request = URLRequest(url: base.appendingPathComponent("api/states/input_text.microsoft_teams_status"))
            request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
            payload["attributes"] = ["friendly_name": "Microsoft Teams Status"]
